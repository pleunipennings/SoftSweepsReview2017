 ...
 *   pleuni@dds.nl
 *   Code for the simulations for Hermisson & Pennings 2017, Methods in Ecology and Evolution, Figure 3B
 *	 Soft Sweeps Review
 ***************************************************************************/
...
void popAdapt(gsl_rng *rng){
		...
		if (t == GenEnvChange) {
			...
			if (deterministic==1){
				for (int l= 0; l<numloci;l++){
					...
					cerr << l << "\t" << popArray[l][1]<< "\t" << popArray[l][0] << "\n";}
			}
	...
	cout << "SGV\t" << SGV << "\tnumberOfSGVCopies[fixed_locus]\t" << numberOfSGVCopies[fixed_locus]<<"\n"; //to get the distribution of allele freqs
	//is it from SGV?
	if (SGV == 0 && DeNovo ==1){
