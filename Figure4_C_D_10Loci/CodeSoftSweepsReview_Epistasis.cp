 ...
 *   pleuni@dds.nl                                
 *   code for the simulations for Hermisson & Pennings 2016
 *	 Soft Sweeps Review
 ***************************************************************************/
...
const int numloci = 10;
const int N = 10000; //popsize, hard coded to be 10000, but can change later
...
unsigned int popArray[numloci][N+1] = {}; //the number of individuals at 10000 alleles, 1 loci
unsigned int popArrayAtEnvChange[10][10001] = {};
unsigned int popArrayLocus[10001] = {}; // an array to use in evolve
int alleleAge[10][10001] = {}; //the ages 10000 alleles, 1 loci
double alleleFitness[10][10001] = {}; //the relative fitnesses 10000 alleles, 1 loci
double popArrayWeights[10001] = {}; //the weights for multinomial sampling. should be num individuals * fitness
int numberOfSGVCopies[10] = {}; //number of SGV copies at the env change

//Other variables
int mut_from_wt=0; //number of new mutants in a generation
double WTfreqprod = 1; //needed to get epistasis
int numMos = 0; int numSos = 0; int numHardDeNovo = 0; int numHardSGV = 0;//to keep track of types of sweeps

const int seedTRY = seed;
...
void popAdapt(gsl_rng *rng){
	...
	while (WTfreqprod >0.05 | t <= GenEnvChange){ //each loop is one generation, continues until one of the WT allele almost dies out
		...
		if (t == GenEnvChange) {
			...
			if (deterministic==1){
				for (int l= 0; l<numloci;l++){
					...
					for (int j = 2; j<(maxnumalleles+1); j++){popArray[l][j]=0;} //store pop state at env change
					cout << l << "\t" << popArray[l][1]<< "\t" << popArray[l][0] << "\n";}
			}
		...
		if (t >= GenEnvChange) {//update the fitnesses of the wt for each locus //Oct 15 2016, we now include epistasis PSP: this should be checked
			...
			if (t == GenEnvChange) {cout << "FreqPheno\t"<< 1-WTfreqprod <<"\t";
				cout<<"WTpopsEnvChange\t";
				for (int l = 0; l < numloci; l++){cout << popArray[l][0]<<"\t";}
			}
	...
	cout << "Gen\t"<< t-GenEnvChange << "\tOutcome\t";
	//is it from SGV?
	if (SGV == 0 && DeNovo ==1){
		if (numcontributingalleles==1) {numHardDeNovo ++; cout << "HDN\n" ;} //PSP these should be un-commented for the main sims
		if (numcontributingalleles>1) {numMos ++; cout << "MOSDN\n" ;} //PSP these should be un-commented for the main sims
	}
	if (SGV == 1 && DeNovo ==1){
		numMos ++; cout << "MOSMX\n" ;}
	if (SGV == 1 && DeNovo ==0){
		if (numcontributingalleles==1){numHardSGV ++; cout << "HSGV\n" ; }//only one SGV copy
		if (numcontributingalleles>1){
			...
			if (numcontributingSGValleles>1) {numMos ++; cout << "SSGVMO\n";}
			if (numcontributingSGValleles==1) {numSos ++; cout << "SSGVSO\n";}
		}
	}
	//cout <<"\n";
}
...
int main (int argc, char *argv[]){
	...
	//cout << "\tHard\t" << numHardDeNovo  << "\tHardSGV\t" << numHardSGV<<"\tSos\t" << numSos << "\tMos\t" << numMos << "\n";
